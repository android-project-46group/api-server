...
func testBlogsExists(t *testing.T) {
	...
	e, err := BlogExists(ctx, tx, o.BlogID)
	if err != nil {
...
func testBlogsFind(t *testing.T) {
	...
	blogFound, err := FindBlog(ctx, tx, o.BlogID)
	if err != nil {
		t.Error(err)
	}
...
func testBlogToOneMemberUsingMember(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadMember(ctx, tx, false, (*[]*Blog)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testBlogToOneSetOpMemberUsingMember(t *testing.T) {
	var err error
...
var (
	blogDBTypes = map[string]string{`BlogID`: `integer`, `MemberID`: `integer`, `BlogURL`: `character varying`, `LastBlogImg`: `character varying`, `LastUpdatedAt`: `character varying`}
	_           = bytes.MinRead
)
