...
func testGroupsExists(t *testing.T) {
	...
	e, err := GroupExists(ctx, tx, o.GroupID)
	if err != nil {
...
func testGroupsFind(t *testing.T) {
	...
	groupFound, err := FindGroup(ctx, tx, o.GroupID)
	if err != nil {
		t.Error(err)
	}
...
func testGroupToManyMembers(t *testing.T) {
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadMembers(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
var (
	groupDBTypes = map[string]string{`GroupID`: `integer`, `GroupName`: `character varying`}
	_            = bytes.MinRead
)
